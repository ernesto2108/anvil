...
grep "last_updated:" .context/NAVIGATOR.md
```
...
| > 7 días | OUTDATED | Recomendar re-scan: "El contexto tiene X días. Sugiero correr el scanner antes de continuar." |
| No existe | NONE | Informar: "No hay `.context/`. Puedes generarlo con `scanner mode: deep`." |
...
- No actualizar el archivo `.context/` en este paso — eso es responsabilidad del reporter al final del run
...
| `bootstrap` | Generado automáticamente por scanner — puede tener gaps |
| `partial` | Bootstrap + algunas actualizaciones manuales |
| `full` | Revisado y completado manualmente por el equipo |

El coverage no determina staleness — un `full` puede estar stale si el código evolucionó sin actualizar `.context/`.
