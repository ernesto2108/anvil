---
name: test-api
description: Smoke testing de endpoints HTTP y de flujos encadenados, en dos modos: escanea los cambios recientes del developer, o prueba lo que el humano indique. Deriva una matriz sistemática de casos a partir del schema (campos, formatos, boundaries, reglas de negocio) con presupuesto por endpoint y sin explosión combinatoria. Genera `tests/api-collection.json` (Postman Collection v2.1) y ambientes en `tests/environments/`, portables e importables en otros clientes y ejecutables con Newman CLI. Soporta flujos multi-paso con extracción de variables, login automático, auth flexible y multi-servicio. Úsalo cuando termines de implementar endpoints, quieras probar APIs existentes, necesites una colección portable, o pidas "probar el flujo de X", "colección con ambientes", "environment de staging", "smoke cross-service", "login automático en la colección". No es para tests unitarios ni suites E2E permanentes.
---
