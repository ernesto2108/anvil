---
name: test-api
description: Smoke testing de endpoints HTTP y de flujos encadenados con dos modos (1) escanea cambios recientes del developer, o (2) el humano indica qué endpoints o flujo probar. Deriva una matriz sistemática de casos por campo y regla del schema (campo ausente, formato inválido, boundary values, tipo incorrecto, enum fuera de rango, reglas de negocio) con presupuesto por endpoint y sin explosión combinatoria. Genera `tests/api-collection.json` en formato Postman Collection v2.1 más archivos de ambiente en `tests/environments/` — importable en Postman, Insomnia, Thunder Client, Hoppscotch y ejecutable con Newman CLI. Soporta flujos multi-paso con extracción de variables, login automático, auth flexible (Bearer, API key, basic, cookie) y multi-servicio (microservicios, una URL por servicio, lectura de service-map.yaml). Úsalo cuando termines de implementar endpoints, quieras probar APIs existentes, necesites una colección portable, o pidas "probar el flujo de X", "colección con ambientes", "environment de staging", "smoke cross-service", "login automático en la colección". No es para tests unitarios ni para suites E2E permanentes.
---
