...
description: Cierre completo de cualquier modo del Líder (Explorador, Planeación, Integración, Pruebas) — resuelve vault path desde project-registry, escribe nota al vault según tipo de cambio, spawnea reporter para aplicar delta a `.project-context/`, ejecuta `/task-complete`, cierra la orquestación en Anvil MCP, actualiza `last_updated` en NAVIGATOR, llama `digest_from_handoff` cuando no corrió reporter, y limpia `.project-context/runs/`. Cárgalo cuando el Líder esté por cerrar cualquier modo. Reemplaza las secciones de cierre y de Persistencia de runs del leader.md.
user-invocable: false
---

# integration-close

Secuencia de cierre que el Líder ejecuta antes de presentar el output final al usuario en **cualquier modo** (Explorador, Planeación, Integración, Pruebas). Cubre las dos responsabilidades acopladas: (a) escritura al vault del proyecto (Regla inviolable #6 del `leader.md`) y (b) persistencia/limpieza del run (Anvil MCP + `.project-context/runs/`).
...
El orden es **obligatorio** — saltarse o reordenar pasos rompe la trazabilidad del run y la consistencia de `.project-context/`.
...
### 6 — Actualizar `last_updated` en `.project-context/NAVIGATOR.md`
...
### 8 — Limpiar `.project-context/runs/<run-id>/`

Si el run cerró en `status="success"` → limpiar el directorio `.project-context/runs/<run-id>/` (es scratchpad temporal — el historial vive en Anvil MCP).
...
| Plan de trabajo activo | `.project-context/runs/<run-id>/plan.md` | Scratchpad local — temporal |
| Outputs intermedios, visual check | `.project-context/runs/<run-id>/` | Solo mientras el run está activo |
| Conocimiento del repo | `.project-context/` (project.md, patterns.md, domains/, contracts.md) | Fuente de verdad — siempre actualizar al cierre |
| Trazabilidad cross-proyecto | Vault del proyecto (resuelto vía `project-registry.md`) | Nota humana del cambio |

`.project-context/runs/` no es historial — es un workspace temporal. El historial vive en Anvil MCP.

## Microservicios

En proyectos cross-service: el run vive en Anvil MCP con referencias a todos los repos tocados. Cada repo actualiza su propio `.project-context/` al cierre vía spawn de `reporter`. El Líder coordina que todos los `reporter` apliquen el delta antes de marcar `success`.
...
- El Líder NUNCA escribe en `.project-context/domains/`, `.project-context/patterns.md`, `.project-context/contracts.md`, `.project-context/ops.md`, `.project-context/risks.md` ni `.project-context/decisions/` — esos paths están en `denied_tools`. Toda escritura ahí pasa por `reporter`.
...
- Si `status != "success"` → NO limpiar `.project-context/runs/<run-id>/`.
