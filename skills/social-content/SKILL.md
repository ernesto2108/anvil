...
Ver `reference.md` sección A para psicología del color, contraste, tipografía y reglas de carruseles en detalle.
...
lText=I(left,{type:"text",content:"3 hours writing one email",fontFamily:"Inter",fontSize:32,fontWeight:"$fw-bold",fill:"#991b1b",textAlign:"center",lineHeight:1.3})
right=I(card,{type:"frame",layout:"vertical",width:"fill_container",height:"fill_container",fill:"#f0fdf4",padding:60,justifyContent:"center",alignItems:"center",gap:24})
rLabel=I(right,{type:"text",content:"AFTER",fontFamily:"Inter",fontSize:20,fontWeight:"$fw-bold",fill:"#166534",opacity:0.5,letterSpacing:0.1,textTransform:"uppercase"})
...
name=I(card,{type:"text",content:"Maria Lopez",fontFamily:"Inter",fontSize:20,fontWeight:"$fw-bold",fill:"#1c1917",opacity:0.6})
...
code=I(block,{type:"text",content:"// Before: N+1 query nightmare\nfor _, user := range users {\n  orders := db.Find(&orders, user.ID)\n}\n\n// After: one query, done\norders := db.Preload(\"Orders\").Find(&users)",fontFamily:"JetBrains Mono",fontSize:22,fill:"#c9d1d9",lineHeight:1.6})
...
Dimensiones, algoritmos detallados (LinkedIn 2025-2026, Instagram, Twitter/X), formatos clasificados por engagement, longitudes óptimas, estrategia de engagement y reglas generales (RGB, padding, zona segura): ver `reference.md` sección B.
...
**Por qué importa:** más del 50% de los posts en LinkedIn son generados por IA. Los posts escritos por humanos superan a los detectados como IA en 40-44% de engagement. Si suena a IA, la gente hace scroll.

**Reglas obligatorias** (ver `reference.md` sección C para listas completas, tablas y checklist de auto-revisión):

- **C.1** — palabras prohibidas (verbos, adjetivos, sustantivos, adverbios, frases de relleno típicas de LLM)
- **C.2** — patrones estructurales prohibidos (regla de tres, "not just X but Y", abuso del guión em, etc.)
- **C.3** — patrones de tono prohibidos (superlativos vacíos, autoridad vaga, vulnerabilidad performativa)
- **C.4** — técnicas de voz humana (ritmo irregular, contracciones, específico sobre general, verbos simples)
- **C.5** — checklist de auto-revisión ANTES de presentar cualquier borrador
- **C.6** — la prueba definitiva: "¿Me daría vergüenza si alguien responde 'nice ChatGPT post'?"

Cargar `reference.md` sección C cada vez que se vaya a redactar o revisar copy.
