...
> ⚠ No spawnear `architect` en el mismo mensaje que `requirements` — esperar el output completo de `requirements` primero.
...
> `requirements → architect` es **siempre secuencial y nunca paralelizable** — el `architect` tiene un hard-stop si `requirements.md` no está en su prompt (Pre-check #6). Lanzarlos en paralelo causa fallo garantizado del `architect`.
