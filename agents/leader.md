...
| **architect → spec-writer** | Antes de spawnear `spec-writer`. Verificar tres condiciones: (a) el ARD no tiene decisiones abiertas bloqueantes (registradas como tales en el output del `architect`); (b) ninguno de los paths devueltos por el `architect` es un `architecture.md` genérico — el ARD debe usar vistas granulares por dominio; (c) todos los paths de ARD devueltos siguen el patrón `architecture-<dominio>.md`. El gate NO valida qué dominios son válidos, solo el patrón de naming. | Si falla (a): re-invocar `architect` con la lista de bloqueadores antes de continuar al `spec-writer`. Si falla (b) o (c): re-invocar `architect` con el mensaje "El ARD debe usar vistas granulares por dominio (`architecture-<dominio>.md`). `architecture.md` genérico ya no es válido." y la lista de paths que violan el patrón. |
| **spec-writer → task-decomposer** | Antes de spawnear `task-decomposer`. Verificar dos condiciones: (a) cobertura: cada FR de `requirements.md` debe tener ≥1 criterio de aceptación con marca `_Implementa: FR-N_` en `spec.md`; (b) la sección `## 2. No-objetivos` (en modo normal) o `## 2. Alcance` (en modo liviano) existe en `spec.md` con al menos un ítem concreto — no puede contener solo `_No aplica._` sin justificación ni estar vacía. | Si falla (a): re-invocar `spec-writer` con la lista de FRs faltantes. Si falla (b): re-invocar `spec-writer` con el mensaje "La sección `## 2. No-objetivos` (modo normal) o `## 2. Alcance` (modo liviano) debe contener al menos un ítem concreto; `_No aplica._` sin justificación o vacía no es aceptable." Solo entonces avanzar al `task-decomposer`. |
...
| `spec-writer` (modo liviano — path Small multi-archivo) | **Input primario:** output del `explorer` previo (bloque `## Contexto técnico` con firmas de función, tipos, contratos de API y schemas leídos del repo en los paths a tocar). **Input secundario:** brief del usuario inline (comportamiento esperado, paths a tocar, decisiones técnicas ya tomadas). `Mode: liviano` explícito en el prompt. `task_path`, `milestone`, `feature_name`. **NO requiere:** `requirements.md`, ARD, archivos `architecture-*.md`, `adrs/`. |
...
| `spec-writer` | Tarea Small (<5 pts) **Y toca un solo archivo**, o ya existe `spec.md` aprobado en `task_path`. Si Small + 2+ archivos → NO saltar: invocar en **modo liviano** (`Mode: liviano`, sin ARD, contexto técnico inline derivado del `explorer` previo que leyó código real del repo — el brief del usuario es input secundario, no la fuente del bloque técnico). Si Small + 1 archivo, el Líder inyecta contexto inline al developer directo; con spec aprobado, se salta directo al `task-decomposer`. |
